...
public class BlockExecutor {
    ...
    public void workReward(String address) throws ApiException {
        ...
        BigDecimal a = new BigDecimal(0.6);// cjs矿工得的奖励比例
        if(blockManager.getLastBlock().getBlockBody().getTransactions().isEmpty()){
            account.setBalance(account.getBalance().add(BigDecimal.valueOf(App.MINE_REWARD)));
            accountManager.putAccount(account);
        }else {
            //cjs交易不为空，奖金分给1.矿工和2.交易发送者

            // cjs 1.分发给矿工
            account.setBalance(account.getBalance().add(BigDecimal.valueOf(App.MINE_REWARD).multiply(a)));
            accountManager.putAccount(account);
            // cjs 2.分发给交易者
            // cjs如果交易不为空,遍历交易并把挖矿金额分给发送者
            List<Transaction> transactions = blockManager.getLastBlock().getBlockBody().getTransactions();
            for (Transaction transaction : transactions) {
                //转账账户
                /*Account address_from = accountManager.getAccount(address);
                //收款账户
                Account address_to = accountManager.getAccount(transaction.getFrom());

                //执行转账操作,更新账户余额
                safeTran(address_from, address_to, BigDecimal.valueOf(App.MINE_REWARD).multiply( BigDecimal.valueOf(1).subtract(a)));*/

                //accountImpl.getBalance(transaction.getFrom());
                Account address_to = accountImpl.getAccount(transaction.getFrom());
                address_to.setBalance(address_to.getBalance().add(BigDecimal.valueOf(App.MINE_REWARD).multiply(BigDecimal.valueOf(1).subtract(a))));
                //更新数据库
                accountManager.putAccount(address_to);
        ...
        // cjs 如果是同步区块，就不显示这条log
        if (syncBlockResponseHandler.getCurrentSyncBlock() == null) {

            if(blockManager.getLastBlock().getBlockBody().getTransactions().isEmpty()){
                logger.info("矿工奖励 " + BigDecimal.valueOf((App.MINE_REWARD)) + " 已存入账户：" + address);
            }else {
                logger.info("矿工奖励 " + BigDecimal.valueOf((App.MINE_REWARD)).multiply(a) + " 已存入账户：" + address);
            }
